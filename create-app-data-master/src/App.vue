  ...
  computed: {
    dataAppCategory: function() {
      if (this.categories && this.categories.length > 0) {
        const categ = this.categories.find(
          category => category.value === parseInt(this.appCategory)
        );
        if (categ) {
          return categ.text;
        }
    ...
    handleClose() {
      if (this.dirtyForm === true) {
        this.$swal({
          title: "Are you sure?",
          text:
            "You have unsaved data. If you leave all data will be lost. Are you sure you want to leave?",
          type: "warning",
        ...
        this.$swal(
          "Canceled",
          "You left without create any new application",
          "info"
        );
      }
    },
